...
OUTPUT_DIR = 'ABC-Ts-NEW'
CSV_DIR = os.path.join(OUTPUT_DIR, 'csv')
LOG_FILE = os.path.join(OUTPUT_DIR, 'chembl_api.log')

# Terms to identify ABC transporters (case-insensitive)
ABC_PATTERNS = ['ABC', 'ATP-BINDING CASSETTE', 'ABCA', 'ABCB', 'ABCC', 'ABCD', 'ABCE', 'ABCF', 'ABCG', 'ABC-T', '*ABC', 'ABC*', '*ABC*']
...
def fetch_all_human_targets():
    """Fetch all human targets from ChEMBL with a higher limit to ensure full retrieval."""
    logging.info("Fetching all human targets from ChEMBL...")
    target = new_client.target
    results = target.filter(organism=SPECIES, limit=100000)
    targets = list(results)
    logging.info(f"Total human targets retrieved: {len(targets)}")
    return targets

def is_abc_transporter(target_data):
    """
    Check if the target is an ABC transporter by looking for any of the ABC_PATTERNS
    in a case-insensitive manner within:
    - pref_name
    - description
    - component synonyms
    """
    name = (target_data.get('pref_name', '') or '').upper()
    desc = (target_data.get('description', '') or '').upper()

    def matches_abc(s: str) -> bool:
        s = s.upper()
        return any(pattern in s for pattern in ABC_PATTERNS)

    # Check in pref_name and description
    if matches_abc(name) or matches_abc(desc):
        return True
    ...
    for comp in components:
        ...
        for syn in synonyms:
            syn_name = (syn.get('component_synonym', '') or '').upper()
            if matches_abc(syn_name):
                return True

    return False

def filter_abc_transporters(all_targets):
    """Filter targets to include only ABC transporters based on the defined patterns."""
    logging.info("Filtering for ABC transporters among all human targets...")
    abc_transporters = [t for t in all_targets if is_abc_transporter(t)]
...
def fetch_activity_data(target_chembl_id, activity_types):
    ...
    Returns a DataFrame of activities including SMILES and pChEMBL values.
    """
    ...
    try:
        ...
        for act in query:
            ...
            # Fetch molecule details for SMILES
            mol_id = activity_info['molecule_chembl_id']
            if mol_id:
                try:
                    mol_data = molecule.get(mol_id)
                    smiles = mol_data.get('molecule_structures', {}).get('canonical_smiles', '')
                    activity_info['smiles'] = smiles
                except Exception as e:
                    logging.error(f"Error fetching molecule {mol_id} details: {e}")
                    activity_info['smiles'] = ''
...
def process_abc_transporters(abc_df):
    ...
    including SMILES and pChEMBL values, and save them as separate CSV files.
    """
...
def main():
    try:
        ...
        if abc_df is not None and not abc_df.empty:
            ...
            print("No ABC transporters found with the given criteria.")
    except Exception as e:
